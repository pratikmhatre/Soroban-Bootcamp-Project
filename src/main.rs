...
struct Ball{ 
    ...
    fn new()-> Game{
         Game{
            ...
            ball : Ball{
                x : 44,
                y : 21,
                vertical : VertDir::Down,
                horizontal : HorizDir::Right
            }
         }
    }
 ...
 impl Ball{
    fn bounce(&mut self, frame : &Frame){
        if self.x <= 0 {
            self.horizontal = HorizDir::Right;
        }
        else if frame.width <= self.x{
            self.horizontal = HorizDir::Left;
        }
        else if self.y <= 0{
            self.vertical = VertDir::Down;
        }else if frame.height <= self.y{
            self.vertical = VertDir::Up;
        }
        else {}
    }
